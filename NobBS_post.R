...
NobBS.post <- function(data, now, units, onset_date, report_date, moving_window=NULL, max_D=NULL, cutoff_D=NULL, proportion_reported=1, quiet=TRUE,
                  specs=list(
                    dist=c("Poisson","NB"),
                    alpha1.mean.prior=0,
  ...
  full.post.n <- matrix(NA, ncol=specs$nSamp, nrow=now.T)
  for(v in t.extract){
    full.post.n[v, ] <- mymod.dat[, grep(paste("sum.n[",v,"]",sep=""), colnames(mymod.dat), fixed=TRUE)]
  }
  
  list(estimates=estimates, estimates.inflated=estimates.inflated, full.post.n=full.post.n)
